...
/// </summary>
public class LeeGridLoader : MonoBehaviour
{
    [Header("Addressables")]
    ...
    [Header("UI")]
    [Tooltip("LeeGridSelectButton 컴포넌트가 부착된 버튼 프리팹.")]
    ...
    private AsyncOperationHandle<IList<LeeGridAssetSO>> _handle;
    ...
    private IEnumerator LoadGridSOs()
    {
        ...
        {
            Debug.LogError("[LeeGridLoader] selectButtonPrefab이 null입니다. Inspector에서 할당하세요.");
            yield break;
        }
        if (buttonContainer == null)
        {
            Debug.LogError("[LeeGridLoader] buttonContainer가 null입니다. Inspector에서 할당하세요.");
            yield break;
        }

        Debug.Log($"[LeeGridLoader] '{gridGroupKey}' 레이블로 GridAssetSO 로드 시작...");

        _handle = Addressables.LoadAssetsAsync<LeeGridAssetSO>(gridGroupKey, null);
        _handleValid = true;
        ...
        {
            Debug.LogError(
                $"[LeeGridLoader] '{gridGroupKey}' 로드 실패!\n" +
        ...
        {
            Debug.LogError(
                $"[LeeGridLoader] '{gridGroupKey}' 레이블로 등록된 GridAssetSO가 없습니다.\n" +
                "Addressables Groups 창에서 각 GridAsset SO의 Labels 컬럼에 'SO Grids'가 있는지 확인하세요.");
            yield break;
        }

        Debug.Log($"[LeeGridLoader] {_handle.Result.Count}개 GridAssetSO 로드 완료. 버튼 생성 시작.");
        ...
        foreach (var gridSO in _handle.Result)
        {
            ...
            var btn = go.GetComponent<LeeGridSelectButton>();
            if (btn != null)
            {
