...
/// <summary>
/// 런타임 데이터 클래스(LeeGridAssetData 등)로부터
/// ScriptableObject.CreateInstance를 통해 SO 인스턴스를 생성한다.
///
/// 생성된 SO는 에셋이 아니므로 씬/세션 종료 시 GC 대상.
/// LeeBoardManager.OnDestroy에서 Destroy를 호출해 명시적으로 해제한다.
/// </summary>
public static class LeeSORuntimeFactory
{
    // ── LeeGridAssetData → SO 세트 생성 ─────────────────────────────

    /// <summary>
    /// LeeGridAssetData 전체로부터 런타임 SO 세트를 생성한다.
    /// 프리팹은 외부에서 주입받는다 (LeeBoardManager의 defaultSquarePrefab 등).
    /// out 파라미터로 하위 SO들을 반환하므로 호출자가 Destroy 책임을 진다.
    /// </summary>
    public static LeeGridAssetSO CreateGridAssetSO(
        LeeGridAssetData data,
        GameObject squarePrefab,
        GameObject shapeBlockPrefab,
        out LeeGridPatternSO patternSO,
        out LeeGridVisualSO visualSO,
        out LeeShapeAssetSO[] shapeSOs)
    {
        // Pattern SO
        patternSO = ScriptableObject.CreateInstance<LeeGridPatternSO>();
        ...
        // Visual SO
        visualSO = ScriptableObject.CreateInstance<LeeGridVisualSO>();
        ...
        {
            shapeSOs = new LeeShapeAssetSO[data.spawnableShapes.Length];
        ...
        // Grid Asset SO
        var so = ScriptableObject.CreateInstance<LeeGridAssetSO>();
        so.name            = data.id;
    ...
    /// <summary>LeeBoardConfigData로부터 런타임 LeeBoardConfigSO를 생성한다.</summary>
    public static LeeBoardConfigSO CreateBoardConfigSO(LeeBoardConfigData data)
    {
        var so = ScriptableObject.CreateInstance<LeeBoardConfigSO>();
        so.startInSelectionMode = data.startInSelectionMode;
        so.gameplayUniformScale = data.gameplayUniformScale;
        return so;
    }

    /// <summary>LeePlacementRulesData로부터 런타임 LeePlacementRulesSO를 생성한다.</summary>
    public static LeePlacementRulesSO CreatePlacementRulesSO(LeePlacementRulesData data)
    {
        var so = ScriptableObject.CreateInstance<LeePlacementRulesSO>();
    ...
    public static void DestroyRuntimeSOs(
        LeeGridAssetSO gridSO,
        LeeGridPatternSO patternSO,
        LeeGridVisualSO visualSO,
        LeeShapeAssetSO[] shapeSOs)
    {
