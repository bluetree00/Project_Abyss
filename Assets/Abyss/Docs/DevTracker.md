...
| 이름 | 브랜치 | 담당 |
|------|--------|------|
| KBG  | `dev/KBG-N` (현재) | 아키텍처, UI, 전반 시스템, 플레이어 |
| Lee  | `dev/lee-SO` (현재) | 몬스터 AI, 그리드 블록 시너지 시스템 |
...
### KBG — `dev/KBG-N`
- 게임 준비 패널(PrepPanel) 완성 → **머지 완료 (2026-03-16)**
- 다음: 런 진행 루프 (방 진입/클리어) 및 인게임 HUD 세부 연동

### Lee — `dev/lee-SO`
- 몬스터 AI 0.3ver 완성 → 머지 완료
- 그리드 블록 시너지 시스템 개발 진행 중
- 참고 문서: `Lee_Abyss.md`
...
> 상세 내용은 `Lee_Abyss.md` 참고. 여기서는 머지 현황만 기록.
...
5.4 ResourceManager — Addressables 전환 후 레거시 제거 여부 결정 필요
