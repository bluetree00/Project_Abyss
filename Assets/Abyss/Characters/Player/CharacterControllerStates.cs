...
namespace Game.CharacterStates.PlayerControllerStates
{
    /// <summary>
    /// 애니메이션 상태를 공통으로 처리하는 추상 클래스.
    /// 특정 애니메이션 이름과 종료 시간(normalizedTime)을 기준으로 상태 전환을 제어한다.
    /// </summary>
    public abstract class AnimationState<T> : State<T> where T : PlayerController
    {
    ...
    /// </summary>
    public class IdleState<T> : State<T> where T : PlayerController
    {
        public override void Enter(T owner)
        {
    ...
    /// 캐릭터가 이동 중일 때의 상태. (걷기)
    /// </summary>
    public class MoveState<T> : State<T> where T : PlayerController
    {
        public override void Enter(T owner)
        {
    ...
    /// </summary>
    public class DodgeState<T> : State<T> where T : PlayerController
    {
        public override void Enter(T owner)
        {
    ...
    /// 캐릭터가 사망할 때의 상태.
    /// </summary>
    public class DieState<T> : State<T> where T : PlayerController
    {
        public override void Enter(T owner)
        {
