...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 드래곤 전용 상태 — 이륙 (Takeoff)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// <summary>
/// 이륙 상태. 지상→공중 전환 애니메이션을 재생한다.
/// UnInterruptible — 피격 시 GetHit 전환 차단, 데미지는 적용됨.
/// 애니메이션이 끝나면(normalizedTime ≥ 0.9) AirChase 로 전환한다.
/// </summary>
public class DragonTakeoffState : SpecialStateBase
{
    ...
    public override void Enter(MonsterContext ctx)
    {
        ...
        var dragon = ctx.Monster as DragonBossMonster;
        string animName = dragon != null ? dragon.TakeoffStateName : "Takeoff";
        _animHash = Animator.StringToHash(animName);

        if (ctx.Animator != null && ctx.Animator.HasState(0, _animHash))
        {
            ctx.Animator.speed = 1f;
            ctx.Animator.CrossFade(animName, 0.1f, 0, 0f);
        }

        if (dragon != null) dragon.DragonBlackboard.IsAirborne = true;
    }
    ...
    private bool IsAnimNearEnd(MonsterContext ctx)
    {
        if (ctx.Animator == null) return true;
        if (!ctx.Animator.HasState(0, _animHash)) return true;
        if (ctx.Animator.IsInTransition(0)) return false;
        var info = ctx.Animator.GetCurrentAnimatorStateInfo(0);
        if (info.shortNameHash != _animHash) return true;
        return info.normalizedTime >= 0.9f;
    }
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 드래곤 전용 상태 — 공중 추적 (AirChase)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// <summary>
/// 공중 추적 상태. NavMesh 없이 Transform 으로 직접 이동한다.
/// UnInterruptible — 공중에서 GetHit 전환 차단.
/// airChaseDuration 초 경과 후 착지 상태로 전환한다.
/// </summary>
public class DragonAirChaseState : SpecialStateBase
{
    public override SpecialStateConstraint Constraints => SpecialStateConstraint.UnInterruptible;
    ...
    {
        var dragon = ctx.Monster as DragonBossMonster;
        _timer = dragon != null ? dragon.AirChaseDuration : 8f;

        string animName = dragon != null ? dragon.AirChaseStateName : "AirChase";
        if (ctx.Animator != null && ctx.Animator.HasState(0, Animator.StringToHash(animName)))
        {
            ctx.Animator.speed = 1f;
            ctx.Animator.CrossFade(animName, 0.2f);
        }
    }
    ...
    {
        var dragon = ctx.Monster as DragonBossMonster;
        float height    = dragon != null ? dragon.AirChaseHeight    : 4f;
        float speedMult = dragon != null ? dragon.AirChaseSpeedMult : 1.4f;
        float speed     = ctx.Stat.moveSpeed * speedMult;

        Vector3 targetPos = ctx.Runtime.PlayerTarget != null
            ? ctx.Runtime.PlayerTarget.position + Vector3.up * height
            : ctx.Transform.position;

        ctx.Transform.position = Vector3.MoveTowards(
            ctx.Transform.position, targetPos, speed * Time.deltaTime);
        ...
        if (ctx.Runtime.PlayerTarget != null)
        {
            ...
            if (dir.sqrMagnitude > 0.001f)
                ctx.Transform.rotation = Quaternion.Slerp(
                    ctx.Transform.rotation, Quaternion.LookRotation(dir), Time.deltaTime * 8f);
        }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 드래곤 전용 상태 — 착지 (Landing)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// <summary>
/// 착지 상태. 공중→지상 전환 애니메이션을 재생한다.
/// UnInterruptible — 착지 중 GetHit 전환 차단.
/// 애니메이션이 끝나면 NavMesh 를 복구하고 지상 상태로 복귀한다.
/// </summary>
public class DragonLandingState : SpecialStateBase
{
    public override SpecialStateConstraint Constraints => SpecialStateConstraint.UnInterruptible;
    ...
    {
        var dragon = ctx.Monster as DragonBossMonster;
        string animName = dragon != null ? dragon.LandingStateName : "Landing";
        _animHash     = Animator.StringToHash(animName);
        _descentSpeed = ctx.Stat.moveSpeed * 2f;
    ...
    public override void Update(MonsterContext ctx)
    {
        ...
        if (pos.y > _targetY + 0.1f)
        {
            pos.y = Mathf.MoveTowards(pos.y, _targetY, _descentSpeed * Time.deltaTime);
            ctx.Transform.position = pos;
        }

        if (!IsAnimNearEnd(ctx)) return;
    ...
    public override void Exit(MonsterContext ctx)
    {
        RestoreNavMesh(ctx);
        var dragon = ctx.Monster as DragonBossMonster;
        if (dragon != null) dragon.DragonBlackboard.IsAirborne = false;
    }
    ...
    private static void RestoreNavMesh(MonsterContext ctx)
    {
