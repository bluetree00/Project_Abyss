...
public class MonsterHPBarManager
{
    ...
    public async UniTask<MonsterHPBar> RequestHPBarAsync(MonoBehaviour monster, int currentHp, int maxHp, Transform headBone, float headOffset = 0.1f)
    {
        var bar = await GetFromPoolAsync();
        if (bar == null)
        {
            Debug.LogError($"[MonsterHPBarManager] HP 바 프리팹 로드 실패. Addressables에 '{AddressableKey}' 키로 등록됐는지 확인하세요.");
            return null;
        }
    ...
    public void ReturnHPBar(MonsterHPBar bar)
    {
        if (bar == null) return;
        bar.Unlink();
    ...
    private async UniTask<MonsterHPBar> GetFromPoolAsync()
    {
        while (_pool.Count > 0)
        {
    ...
    private async UniTask<MonsterHPBar> CreateNewBarAsync()
    {
        ...
        return go.GetOrAddComponent<MonsterHPBar>();
    }
