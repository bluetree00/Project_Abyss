...
public class BKSpinSlashState : FullLockState<BlackKnightPatternData>
{
    ...
    public override void Enter(LeeMonsterContext ctx)
    {
        ...
        // 경고 장판: 차지 구간(20%)동안만 표시
        _indicator?.ShowCircle(ctx.Transform, Data.spinRadius, Data.spinDuration * 0.2f);
    ...
    public override void Update(LeeMonsterContext ctx)
    {
        ...
        float spinStart = Data.spinDuration * 0.2f;
        float spinEnd   = Data.spinDuration * 0.8f;
    ...
    private float CalcSpinRadius(float elapsed, float spinStart, float spinEnd)
    {
        ...
        {
            Data.spinRadius,
            Data.spinRadius * 1.5f,
            Data.spinRadius * 1.9f,
            Data.spinRadius * 2.4f,
        };
    ...
    private Color CalcTint(float elapsed, float spinStart, float spinEnd)
    {
        ...
        if (elapsed < spinStart)
        {
            ...
            float freq  = Mathf.Lerp(4f, 18f, t);                    // Hz 증가
        ...
        else if (elapsed < spinEnd)
        {
            ...
            float pulseFreq  = Mathf.Lerp(6f, 24f, progress);
        ...
        else
        {
            ...
            float fadeDuration = Data.spinDuration * 0.2f;
    ...
    // ── AoE 데미지 ───────────────────────────────────────
    private void DealAoeDamage(LeeMonsterContext ctx)
    {
        ...
        foreach (var col in colliders)
        {
            ...
            if (rb != null)
            {
                ...
                dir.y = 0.4f;
                rb.AddForce(dir.normalized * ctx.Config.stat.knockbackForce * 1.5f, ForceMode.Impulse);
            }
        }
    }
