...
{
/// <summary>
/// DeathKnight 3회 동심원 피라미드 슬래시 (Pyramid Slash / Attack4) 패턴.
///
/// 흐름:
///  Enter       → 즉시 Attack4 재생
...
/// </summary>
[CreateAssetMenu(menuName = "RelicFairy/Boss/DeathKnight/DK_PyramidSlashPattern", fileName = "DK_PyramidSlashPattern")]
...
public class DKPyramidSlashState : FullLockState<DKPyramidSlashPatternSO>
{
    ...
    // Hit 페이즈: 링별 순차 발동 목록
    private struct RingEntry { public float fireTime; public int ring; public bool isPattern2; public int setIndex; }
    ...
    public override void Enter(MonsterContext ctx)
    {
        ...
        _bossCell   = DKBossRoomContext.WorldToCell(anchor != null ? anchor.position : ctx.Transform.position);
    ...
    private void BuildRingList()
    {
        ...
        // 세트1: 짝수 링 (Pattern1)
        foreach (int r in evens) { _ringList.Add(new RingEntry { fireTime = t, ring = r, isPattern2 = false, setIndex = 0 }); t += Data.ringStep; }
        t += Data.hitGap;

        // 세트2: 홀수 링 (Pattern2)
        foreach (int r in odds) { _ringList.Add(new RingEntry { fireTime = t, ring = r, isPattern2 = true, setIndex = 1  }); t += Data.ringStep; }
        t += Data.hitGap;

        // 세트3: 짝수 링 다시 (Pattern1)
        foreach (int r in evens) { _ringList.Add(new RingEntry { fireTime = t, ring = r, isPattern2 = false, setIndex = 2 }); t += Data.ringStep; }
    ...
    private void UpdateHitPhase(MonsterContext ctx)
    {
        while (_ringIdx < _ringList.Count && _timer >= _ringList[_ringIdx].fireTime)
        {
            ...
            if (_ringIdx == 0 || _ringList[_ringIdx - 1].setIndex != e.setIndex)
                // 세트(공격)마다 Sword Slash 15 이펙트가 스폰되는 시점에 맞춰 재생. 클립 앞 무음 구간은 건너뛰어 0.5초부터 재생
    ...
    private void SpawnBarrier(int maxRing)
    {
        ...
        _barrierRoot = new GameObject("DK_PyramidSlashBarrier");
