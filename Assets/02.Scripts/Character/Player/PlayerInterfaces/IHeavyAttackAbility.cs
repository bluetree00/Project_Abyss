...
public interface IHeavyAttackAbility<T> where T : PlayerCharacter
{
    /// <summary>
    /// 강공격 차지를 시작할 때 호출.
    /// </summary>
    /// <param name="character">강공격을 수행하는 캐릭터</param>
    void HeavyAttackStartCharging(PlayerCharacter character);
    ...
    void HeavyAttackUpdateCharging(PlayerCharacter character, float chargeTime);
    ...
    void HeavyAttackReleaseChargedAttack(PlayerCharacter character, float chargeTime);

    /// <summary>
    /// 강공격이 끝나거나 중간에 취소되었을 때 호출되어, 초기화 작업을 수행.
    /// </summary>
    /// <param name="character">강공격을 수행하던 캐릭터</param>
    void HeavyAttackCancelCharging(PlayerCharacter character);
}
