...
namespace Artngame.Oceanis
{
    public class WavesCascade
    {
                            ...
                            Texture2D gaussianNoise)
        {
            ...
            //v0.9
            if (initialSpectrum == null || size != initialSpectrum.width)
            {
        ...
        public void Dispose()
        {
            ...
            initialSpectrum?.DiscardContents();
            precomputedData?.Release();
            precomputedData?.DiscardContents();
            displacement?.Release();
            displacement?.DiscardContents();
            derivatives?.Release();
            derivatives?.DiscardContents();
            turbulence?.Release();
            turbulence?.DiscardContents();

            buffer?.Release();
            buffer?.DiscardContents();
            DxDz?.Release();
            DxDz?.DiscardContents();
            DyDxz?.Release();
            DyDxz?.DiscardContents();
            DyxDyz?.Release();
            DyxDyz?.DiscardContents();
            DxxDzz?.Release();
            DxxDzz?.DiscardContents();
        }
