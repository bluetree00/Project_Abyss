...
namespace Artngame.CommonTools.BoidsCompute
{
    public class ComputeBoidManager : MonoBehaviour
    {
        ...
        public float maxBoidsHeight = 0;

        public bool timeOptimize = false;
        public float updatePerSecs = 0.02f;
        ...
        public void Update()
        {
            if (!useLateUpdate)
            {
                if (!timeOptimize || (Time.fixedTime - lastUpdateTime > updatePerSecs))
                {
                    UpdateA();
                ...
                {
                    Graphics.DrawMeshInstancedIndirect(boidMesh, 0, boidMat, new Bounds(transform.position, GraphicsIndirectBounds), argsBuffer, 0, null, shadowMode, false);
                }
            }
        }
        ...
        private void SetupCompute()
        {
            ...
            computeShader.SetFloat("maxBoidsHeight", maxBoidsHeight);

            computeShader.SetVector("boidsManagerPos", transform.position);
        ...
        private void BufferUpdateObstaclesAndTargetsNewPos()
        {
            ...
            {
                targetsPos[i] = targets[i].position - this.transform.position;
            }

            for (int i = 0; i < obstacles.Length; ++i)
            {
                obstaclesPos[i] = obstacles[i].position - this.transform.position;
            }
